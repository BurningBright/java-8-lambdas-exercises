...
public class RatioScaleStrategy implements PictureScaleStrategy {
    ...
    public RatioScaleStrategy(InputStream imageStreamIn, OutputStream imageStreamOut) {
    ...
    public void scale(InputStream imageStreamIn, OutputStream imageStreamOut) {
        ...
        try {
            BufferedImage src = ImageIO.read(imageStreamIn); // 读入文件
            ...
            g.dispose();
            ImageIO.write(tag, fmt, imageStreamOut);// 输出到文件流
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
